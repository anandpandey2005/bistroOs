...
export const RegisterUserInputSchema = z.object({
    ...
    phone: z.object({
        ...
        number: z.string().nullable().default(null).refine(
            (v) => v === null || /^\d{10}$/.test(v),
            { message: "Please enter a valid 10-digit phone number" }
        ),
    }),
    password: z.string().min(6, "Password must be between 6 - 10 characters.").max(10, "Password must be between 6 - 10 characters.").trim(),
