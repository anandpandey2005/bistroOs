...
export async function delete_address(req: Request, res: Response): Promise<void> {
    try {
        ...
        if (!address_ids || !Array.isArray(address_ids) || address_ids.length === 0) {
            res.status(400).json({ success: false, message: "We expect adrees that you wanna to delete." });
            return;
        }
        ...
        if (error instanceof Error) {
            res.status(500).json({ success: false, message: error.message || "An Unexpected system error occured" });
        }
