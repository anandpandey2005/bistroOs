...
export async function request_profile_data(req: Request, res: Response): Promise<void> {
  try {
    ...
    const account = await Account.findById(userId).select(
      '-otp -otpExpiry -deacticated -deactived -savedAddress -blocked -device -recentlyViewed',
    );
